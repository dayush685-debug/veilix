...
export function Admin() {
  ...
  // Derived rather than stored. `load` runs from an effect, and setting a
    ...
    <div className="mx-auto max-w-4xl px-4 py-10">
        ...
        {data.engine_failures.length === 0 ? (
          ...
          <>
            ...
            <p className="mt-2 text-xs text-[var(--text-muted)]">
              ...
              search touched has no row here — its health is{' '}
              <em>unknown</em>, not good.
            </p>
          </>
        )}
      </Section>
          ...
          <ul className="flex flex-wrap gap-1.5">
            {Object.entries(data.engines_contributing)
              ...
              .map(([engine, count]) => (
                ...
                >
                  {engine}{' '}
                  <span className="text-[var(--text-muted)]">{count.toFixed(0)}</span>
                </li>
              ))}
          </ul>
        )}
      ...
      <Section title="What this page cannot show">
        <p className="text-sm leading-relaxed text-[var(--text-secondary)]">
          ...
          addresses here, and no way to add one without changing what the
          system collects. Every figure above is an aggregate already present in
          the metrics endpoint, and no metric carries a user-derived label — so
          an operator can see that the system is healthy and cannot see what
          anyone searched for.
        </p>
      </Section>
...
function Tile({
  ...
  return (
    <div className="rounded-[var(--radius-card)] border border-[var(--border-subtle)] bg-[var(--surface-raised)] p-4">
      ...
      {hint && <p className="mt-0.5 text-2xs text-[var(--text-muted)]">{hint}</p>}
    </div>
  );
}

...
export function Privacy() {
  return (
    <article className="mx-auto max-w-2xl px-4 py-10">
      <h1 className="text-2xl font-semibold tracking-tight">What Veilix knows</h1>
      ...
      <Section title="Never stored">
        <Table
          rows={[
            ['Your IP address', 'Never written to disk or to the cache. See below.'],
            ['Your search queries', 'Held in memory for one request. Never persisted with any identifier.'],
            ['Search history', 'Not collected. There is no table, key, or file for it.'],
      ...
      <Section title="Stored, briefly">
        <Table
          rows={[
            ['Rate-limit counters', 'Keyed by a rotating hash, expires in minutes. See below.'],
            ['Cached results', 'Keyed by the query alone, with no identity. Short expiry, and can be turned off.'],
            ['Aggregate metrics', 'Request counts and latency. Counters only — never per-user series.'],
            ['Operational logs', 'Timestamp, route, status, duration. No query text, no IP.'],
          ]}
        />
      ...
      <Section title="What upstream search engines see">
        ...
        <Caveat>
          ...
          searched for X” are the same statement to anyone watching the
          upstream side. Meta-search hides you in a crowd, and a private
          instance has no crowd.
        </Caveat>
      </Section>
      ...
      <Section title="Images">
        <p className="text-sm leading-relaxed text-[var(--text-secondary)]">
          ...
          browser would connect to each one and hand it your address for
          content you never chose to load from them. Veilix fetches thumbnails
          server-side instead, so those hosts see the instance. The cost is
          this server's bandwidth, which is the right side of that trade.
        </p>
      </Section>
      ...
      <Section title="The cache, and the one thing it leaks">
        ...
        <Caveat>
          ...
          who. A short expiry limits the window; it does not remove the
          channel, and no design keeps a shared cache and removes it entirely.
        </Caveat>
      </Section>
  ...
  return (
    <dl className="divide-y divide-[var(--border-subtle)] overflow-hidden rounded-[var(--radius-card)] border border-[var(--border-subtle)]">
      {rows.map(([term, detail]) => (
        <div key={term} className="grid gap-1 px-4 py-3 sm:grid-cols-[minmax(0,14rem)_1fr] sm:gap-4">
